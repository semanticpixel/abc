...
2. Spawn the existing **`abc:reviewer`** subagent (`Agent` tool, `subagent_type: abc:reviewer` — plugin agents register namespaced; the bare `reviewer` does not resolve in a live session) — do **not** edit `agents/reviewer.md`; extend its input via the prompt. Pass:
   ...
   - **Full files for every touched path** — Linear children resolve to a **local checkout** at the Phase 0.7-cached `<workdir>`. **Before** offering it for local reads, assert the checkout is on the reviewed commit: `git -C <workdir> rev-parse HEAD` must equal `<reviewed-sha>` from step 1.
     - **On match** — state the absolute path in the prompt ("repo root for all paths in this diff: `<abs-path>`") and tell the reviewer it may read full files there for surrounding context, rather than working from the hunk alone.
     - **On mismatch** (the ship worker checked out a different commit, the branch advanced locally, etc.) — do **not** let the reviewer read local bytes that don't match the reviewed diff. Either abort the pass with `[head-moved]` (consistent with step 1, drop no marker) or fall back to fetching each file via `gh api /repos/<owner>/<repo>/contents/<path>?ref=<reviewed-sha>` like the `-gh` sibling. Otherwise step 5 would write a marker claiming `<reviewed-sha>` against bytes the review never saw — silently defeating the SHA-pinning guarantee on the Linear path.

     (This local-read path is the GitHub variant's one divergence: there every file is fetched via `gh api ?ref=<reviewed-sha>` unconditionally, because a cross-repo child may have no checkout — so the `-gh` side reads pinned bytes by construction and needs no HEAD assertion.)
