...
Mirror `/abc:ship-issue` Phase 0.5's cron-entry match rule exactly:

- A `CronList` entry matches when its command string contains `/abc:ship-epic <raw-arg>` followed by a word boundary (next char is not alphanumeric, `-`, or `,`).
- If a matching entry exists → no-op, proceed to Phase 1 (the common path on loop-triggered wakes).
- If no match → invoke `Skill(skill: "loop", args: "10m /abc:ship-epic <raw-arg>")` to arm the cron. Then proceed to Phase 1 — the first wake also does the work of the first iteration.
