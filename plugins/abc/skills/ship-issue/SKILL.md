...
4. **Gates pass** → `git push --force-with-lease`. Post a `<!-- ship-issue:rebase:auto -->` marker comment with a one-line note (e.g. *"Rebased on top of #N; checks green; continuing."*) and re-enter Phase 3 on the next wake.
