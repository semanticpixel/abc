...
- For any open PR, gather its **reviewer feedback across all three GitHub comment surfaces** — a reviewer (including `/abc:review-epic-gh` and `abc:reviewer`, which post via the reviews API) may use any of them, so reading only inline comments silently drops feedback and mis-derives `pr-open`:
  ...
  **Union** the three into the PR's **review-comment set**, then apply both filters: author ∉ {the skill, the PR author} **and** timestamp (`created_at` / `submitted_at`) **strictly after** the last skill commit (the `<!-- ship-issue:commit -->` marker lookup). **Exclude** the skill's own `<!-- ship-issue:* -->` comments, the reviewer's `<!-- review-epic:* -->` marker-only comments, and inline replies the skill itself authored (`in_reply_to_id` set on a skill-authored body) — these are bookkeeping, not feedback. This filtered union is exactly what rows 3 / 3a / 4 mean by "review comments." Also fetch its check-runs via `gh pr checks <pr-num> --repo <owner>/<repo> --json name,state,bucket` and its **behind-base signal** via `gh pr view <pr-num> --repo <owner>/<repo> --json mergeStateStatus` (a value of `BEHIND` means the branch is behind its base — Phase 3.5's rebase trigger reads this).
