...
2. **Merge-nudge (idempotent, one-time).** If the PR has been **green-but-unmerged with review addressed** (all checks `bucket=pass`/`skipping`, no unresolved review comments) for **N=5 consecutive `pr-open` wakes** (~30 min at the 6-minute cadence), and no `<!-- ship-issue:note:merge-nudge -->` marker comment already exists on the issue, post one marker-only comment: `<!-- ship-issue:note:merge-nudge -->` (body is the marker plus the one-line "PR green & review addressed — ready to merge"). Count consecutive green `pr-open` wakes by reading prior `<!-- ship-issue:event:* -->` / wake markers — the marker's own presence makes this a no-op on every later wake, so it never re-posts.
3. Return.
