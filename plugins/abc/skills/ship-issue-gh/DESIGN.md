...
12. **Rebase against base is attempt-and-gate; mechanical failures escalate to `blocked-user`, not `failed`.** Prior versions had a single hard-stop: any non-trivial rebase failure → `failed: rebase-conflict-needs-human`. That was too brittle for parallel epic runs where most conflicts are textual (parallel workers add imports to the same file, JSX elements to the same component, etc.) and resolvable by git's three-way merge. New rule: attempt `git rebase origin/<base>`, then run the project's local gates, and only escalate on (a) conflict markers (`blocked-user: rebase-needs-human`) or (b) red gates after a clean rebase (`blocked-user: rebase-clean-but-tests-failed`). Semantic shift: `failed` is reserved for self-cheating and hard correctness walls; mechanical rebase trouble is **recoverable by re-running `/abc:ship-issue-gh <same-arg>` (or `/abc:ship-issue <same-arg>`) after resolving** — not by the cron staying armed. **CronDelete fires on ALL halts** (blocked-user, failed, all-merged): a `blocked-user` halt cancels the loop just like the terminal states, so re-running the command after the human resolves the conflict is what re-arms it. The self-cheating hard stop still applies *inside* the auto-resolve flow — a clean rebase that only goes green by deleting an assertion is the cheat, not the fix. The legacy `rebase-conflict-needs-human` reason string is removed.

13. **This skill never merges; a human merges.** The worker drives the PR to green-and-reviewed and waits. It never runs `gh pr merge`. After N=5 consecutive merge-ready `pr-open` wakes it posts a one-time `<!-- ship-issue:note:merge-nudge -->` marker comment, then keeps waiting. The merge action is always human.

14. **Cancellation is a standalone `cancel` issue comment.** Phase 3's cancel scan (case-insensitive, whole-comment-body or first line) derives `failed: cancel-requested` → CronDelete. `/loop cancel` remains the manual override.

## Known design tensions

Live interactions between the documented procedure and GitHub's native behavior that the current contract does not paper over. Observed in practice — each entry names the workaround currently expected.

### `Closes` trailer races the `blocked-verify` handler on validation-gated PRs

**The interaction.** Phase 4's `pending → implementing` step 7 mandates that the PR body include `Closes <owner>/<repo>#<n>`. GitHub treats the trailer as authoritative — when the PR merges, GitHub auto-closes the linked issue with `stateReason=completed` *before* the next worker wake runs. Phase 3 row 1a is designed to catch this: `state=closed AND stateReason=completed` + `## Validation` heading in body + no `<!-- ship-issue:verify:passed -->` comment → `blocked-verify`. But the `blocked-verify` handler can only inline the validation steps as a comment and surface them for a human — it does not reopen the issue, because the worker contract treats a closed issue as terminal-from-the-skill's-perspective. Net effect: on a PR carrying both a `Closes` trailer and a `## Validation` heading on the linked issue, the issue moves `pr-open → closed (auto)` with the validation step technically present in the comment timeline but never enforced as a gating signal — the issue is already Done from any dashboard's perspective by the time the worker's `blocked-verify` post lands.

**Resolution (now in the skill).** Phase 4's `pending → implementing` step 7 detects a `## Validation` heading in the linked issue (same heading-match rule as Phase 3 row 1a) and, when present, uses **`Refs <owner>/<repo>#<n>`** instead of `Closes ...`. `Refs` links the PR to the issue without auto-closing it on merge, so the issue stays open past merge; the next wake derives `blocked-verify` and the gate is real. The `merged` handler then calls `gh issue close --reason completed` itself after observing a `<!-- ship-issue:verify:passed -->` comment — the worker drives the close on its own timing, after verification. (Previously this was a manual workaround — "omit the `Closes` trailer by hand"; it is now the documented Phase 4 behavior. See [open questions](#open-questions) item 6, resolved.)

**Real case.** Observed during `/abc:ship-issue-gh semanticpixel/theluistorres#21` (ST-9 Cutover, PR #33, May 2026). The issue body had a `## Validation` section. PR #33 included `Closes semanticpixel/theluistorres#21` per the spec. On merge, the issue auto-closed with `stateReason=completed`. The worker's next wake derived `blocked-verify`, posted the validation steps as a comment, and halted — but the issue was already closed, so the epic-status view treated it as Done. No human ever ran the validation steps as a gate; they ran them as a post-mortem.
...
6. ~~**Skill-level auto-omit of the `Closes` trailer on validation-gated PRs.**~~ **Resolved (ST-2):** Phase 4 step 7 now detects a `## Validation` heading in the linked issue and uses `Refs <owner>/<repo>#<n>` (non-closing) instead of `Closes ...`, so the worker — not GitHub's auto-close — drives the issue close after the verify gate passes. The accepted cost is the heading parse + conditional reference choice in `pending → implementing`. See [Known design tensions](#known-design-tensions).
