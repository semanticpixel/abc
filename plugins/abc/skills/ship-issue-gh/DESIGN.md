...
**Workaround.** For cutover-style PRs whose linked issue carries a `## Validation` gate, **omit the `Closes` trailer**. The `merged` handler in Phase 4 calls `gh issue close --reason completed` itself after observing a `<!-- ship-issue:verify:passed -->` comment, so the worker still drives the close — just on its own timing, after verification, not via GitHub's auto-close. Phase 4 step 7 currently mandates `Closes`, so this is a manual deviation from the documented procedure today. A skill-level fix (auto-omit when the linked issue has a `## Validation` heading) is the candidate next step — see [open questions](#open-questions) item 6.
...
6. **Skill-level auto-omit of the `Closes` trailer on validation-gated PRs.** The tension documented in [Known design tensions](#known-design-tensions) above leaves the workaround to user discipline. A skill-level fix — Phase 4 step 7 detects a `## Validation` heading in the linked issue and skips the trailer — eliminates the gate-bypass without manual care. The cost is added code complexity in `pending → implementing` (heading parse, conditional trailer omission) and slightly less-obvious PR bodies for the validation-gated case. Punted unless the manual workaround proves error-prone in practice; revisit if the same gate-bypass shows up on a second project.
