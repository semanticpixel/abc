...
13. **This skill never merges; a human merges.** The worker drives the PR to green-and-reviewed and waits. It never runs `gh pr merge`. Once the last skill commit has been merge-ready for ~30 min — the skill is stateless and keys off that commit's age (its `%ai` timestamp), not a count of `pr-open` wakes — it posts a one-time `<!-- ship-issue:note:merge-nudge -->` marker comment, then keeps waiting. The merge action is always human.
