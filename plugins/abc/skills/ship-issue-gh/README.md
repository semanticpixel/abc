...
- **UI verification is out of scope.** If an issue body has a `## Validation` section, the skill transitions to `blocked-user` post-merge (state: `blocked-verify`) and inlines the validation text. The issue stays open until a human confirms. Auto-verification is planned for Phase C (`/verify-ticket`).
