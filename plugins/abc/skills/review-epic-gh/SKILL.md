...
1. List candidate PRs: for each **open** child, `closedByPullRequestsReferences` plus `gh pr list --repo <owner>/<repo> --state open --json number,headRefName,headRefOid,url` filtered to branches matching `<child-n>-*`.
   ...
   **0 matches from both sources** — the `<child-n>-*` pattern is the *worker's* branch derivation; a human (or a fix-up branch) may have named the real branch differently, and a PR without a `Closes` trailer never appears in `closedByPullRequestsReferences`. Fall back to the child issue's timeline cross-references: `gh api /repos/<owner>/<repo>/issues/<child-n>/timeline --paginate --jq '.[] | select(.event=="cross-referenced") | .source.issue | select(.pull_request != null) | .number'`, resolving each hit via `gh pr view <num> --repo <owner>/<repo> --json number,state,headRefOid,url`. Only when this also misses is the child `[no-pr-yet]`.
